export default {
  slots: {
    ...
    content:
      'fixed bg-[var(--ui-bg)] ring ring-[var(--ui-border)] flex focus:outline-none',
  ...
  variants: {
    direction: {
      top: {
        content:
          'top-0 mb-24 flex-col-reverse rounded-b-[calc(var(--ui-radius)*2)]',
        handle: 'mb-4'
      },
      right: {
  ...
  compoundVariants: [
    {
      direction: ['top' as const, 'bottom' as const],
      class: {
    ...
    {
      direction: ['right' as const, 'left' as const],
      class: {
        content:
          'inset-y-4 w-auto max-w-[calc(100%-2rem)] rounded-[calc(var(--ui-radius)*2)] after:hidden',
