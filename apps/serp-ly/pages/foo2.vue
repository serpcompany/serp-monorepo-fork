                    ...
                    <div class="p-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <UFormField label="URL" required class="col-span-2">
                                <UInput v-model="editLinkData.url" placeholder="https://example.com" />
                            </UFormField>
                            ...
                            <UFormField label="Description" class="col-span-2">
                                <UTextarea v-model="editLinkData.description" placeholder="Link Description" rows="3" />
                            </UFormField>
                            <UFormField label="Comment" class="col-span-2">
                                <UTextarea v-model="editLinkData.comment" placeholder="Personal Comment" rows="2" />
                            </UFormField>
                            <UFormField label="Image URL" class="col-span-2">
                                <UInput v-model="editLinkData.image" placeholder="https://example.com/image.jpg" />
                            </UFormField>
                        </div>
                        <div class="mt-4 flex space-x-2 justify-end">
                            <UButton variant="outline" @click="cancelEdit">Cancel</UButton>
                            <UButton color="primary" :loading="loadingUpdate" @click="updateLink">Save</UButton>
                        </div>
                    </div>
                </UCard>
                    ...
                    <div class="p-4">
                        ...
                        <div v-else>
                            <div v-if="links.length">
                                <div v-for="(link, index) in links" :key="link.slug"
                                    ...
                                    <div class="space-x-2">
                                        ...
                                        <UButton variant="outline" color="danger" size="sm"
...
async function deleteLink(slug: string) {
    try {
        const { data, error } = await useFetch('/api/link/delete', {
            method: 'POST',
            headers: useRequestHeaders(['cookie']),
            body: { slug }
        });
...
function cancelEdit() {
    editLinkData.value = { url: '', slug: '', comment: '', title: '', description: '', image: '', originalSlug: '' };
}
...
async function updateLink() {
    ...
    try {
        const { data, error } = await useFetch('/api/link/update', {
            method: 'PUT',
            headers: useRequestHeaders(['cookie']),
