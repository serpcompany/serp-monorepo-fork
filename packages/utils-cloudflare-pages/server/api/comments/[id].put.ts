...
export default defineEventHandler(async (event) => {
    try {
        ...
        const { commentIndex, comment, timestamp, parentIndices, module } = await readBody(event);
        if (commentIndex === undefined || !comment || !timestamp || !module) {
            return { status: 400, message: '`commentIndex`, `comment`, `timestamp`, and `module` are required' };
        }
    ...
    `;
        ...
        const emailCondition = eq(sql.raw(`json_extract(comments, '${finalPath}.email')`), email);
